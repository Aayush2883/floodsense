...
const sensorIcon = (s) => {
  ...
  return L.divIcon({
    ...
    html: `<div style="display:flex;align-items:center;gap:3px">
      <span style="width:14px;height:14px;border-radius:50%;background:${col};border:2px solid #fff;box-shadow:0 0 0 ${s.alert === 'DANGER' ? 6 : 0}px ${col}33"></span>
      <span style="background:${col};color:#fff;font:600 10px 'IBMPlexMono_500Medium',ui-monospace,monospace;padding:2px 5px;border-radius:4px;white-space:nowrap">${Math.round(s.waterLevelCm)} cm</span></div>`,
  });
};
...
export default function FloodMap({
  ...
  return (
    <View style={[{ flex: 1, overflow: 'hidden', backgroundColor: '#E6EAE3' }, style]}>
