...
export default function AlertBanner({ onGetToSafety }) {
  ...
  return (
    <Animated.View style={[st.wrap, { top: insets.top + 8, transform: [{ translateY: y }] }]}>
      <View style={st.head}>
        <View style={st.appIcon}><Icon name="waves" size={12} color="#fff" /></View>
        <Text style={st.app}>FLOODSENSE</Text>
        <Text style={[st.app, { marginLeft: 'auto' }]}>{lang === 'hi' ? 'अभी' : 'now'}</Text>
        <Pressable onPress={close} hitSlop={10}><Icon name="close" size={16} color={C.muted} /></Pressable>
      </View>
      <Text style={st.title}>{`${t('sevDANGER')}${dist ? ` · ${dist}` : ''}`}</Text>
      <Text style={st.msg} numberOfLines={3}>{banner.message}</Text>
      <View style={st.acts}>
        <Pressable style={[st.act, { backgroundColor: C.safe }]} onPress={() => { close(); onGetToSafety(); }}>
          <Text style={[st.actText, { color: '#fff' }]}>{t('getToSafety')}</Text>
        </Pressable>
...
const st = StyleSheet.create({
  wrap: { position: 'absolute', left: 10, right: 10, zIndex: 100, backgroundColor: 'rgba(255,255,255,0.97)', borderRadius: 18, padding: 12, gap: 3, borderWidth: 1, borderColor: '#EDB3BA', shadowColor: '#0F1E24', shadowOpacity: 0.28, shadowRadius: 20, shadowOffset: { width: 0, height: 8 }, elevation: 12 },
  head: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  appIcon: { width: 18, height: 18, borderRadius: 5, backgroundColor: C.river, alignItems: 'center', justifyContent: 'center' },
  app: { fontFamily: F.bodySemi, fontSize: 11, color: C.muted, letterSpacing: 0.4 },
  title: { fontFamily: F.bodyBold, fontSize: 15, color: C.danger, marginTop: 2 },
  msg: { fontFamily: F.body, fontSize: 13.5, lineHeight: 19, color: C.ink },
  ...
  actText: { fontFamily: F.bodyBold, fontSize: 13, color: C.ink },
});
